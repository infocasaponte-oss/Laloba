...
export async function requireUserId(bearerToken?:string):Promise<string>{
  if(!authConfigured&&!gateIdentityEnabled()){if(databaseConfigured)throw new Error("Auth is disabled while DATABASE_URL is set");return DEV_USER_ID}
  const user=await getSessionUser(bearerToken);if(!user)throw new UnauthorizedError();return user.id;
}
