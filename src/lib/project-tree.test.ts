...
test("project tree identity is deterministic and path-sensitive",async()=>{
 const a=projectTreeFromGeneration("generation_tree_3",{schemaVersion:"2",summary:"x",files:[{path:"src/b.ts",content:"b"},{path:"src/a.ts",content:"a"}]});
 ...
 const renamed=replaceProjectTreeGeneration(a,"generation_tree_5",[{path:"src/c.ts",content:"a"},{path:"src/b.ts",content:"b"}]);
 assert.notEqual(await projectTreeIdentity(a),await projectTreeIdentity(renamed));
});

test("rejects invalid or duplicate files while constructing project trees",()=>{
 assert.throws(()=>projectTreeFromGeneration("generation_tree_6",{schemaVersion:"2",summary:"x",files:[{path:"../secret",content:"x"}]}));
 assert.throws(()=>replaceProjectTreeGeneration({schemaVersion:"2",generationId:"generation_tree_6",files:[]},"generation_tree_7",[{path:"src/a.ts",content:"1"},{path:"src/a.ts",content:"2"}]),/Duplicate path/);
});
