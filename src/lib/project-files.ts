...
export type ProjectSnapshot={schemaVersion:"1";generationId:string;createdAt:string;files:ProjectFile[]};
...
export async function verifyProjectSnapshot(snapshot:ProjectSnapshot){for(const file of snapshot.files)if(await sha256(file.content)!==file.sha256)return false;return true}
