...
describe("isLoginRequired", () => {
  it("is true only for login-required failures", () => {
    ...
    assert.equal(isLoginRequired({ ok: true, data: {}, loginRequired: true } as CallToolResult), false);
  });
});
...
describe("redirectToLoginIfRequired", () => {
  it("no-ops when login is not required", () => {
    ...
    const did = withWindow(
      { location: { assign: (u) => { target = u; }, href: "https://my-app.grok.me/current" } },
    ...
    const did = withWindow(
      {
        ...
        location: { assign: (u) => { assigned = u; }, href: "https://my-app.grok.me/current" },
      },
      () => redirectToLoginIfRequired({
        ok: false,
        data: null,
...
describe("classifyCallToolError", () => {
  ...
  it("falls back to a generic error with the raw message", () => {
    const state = classifyCallToolError({ ok: false, data: null, errorMessage: "boom" });
    assert.equal(state?.kind, "error");
